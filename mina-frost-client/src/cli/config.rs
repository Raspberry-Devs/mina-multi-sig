// The `Group` struct has an `partipant` field which is not used here but in other files
// So we need to allow unused_assignments to avoid clippy warnings about it. The clippy warning
// seems to be generated by the zeroize(skip) macro.

...
//! It tests if the results of frost-client agrees with `mina-signer` (cross-package test)
//! It uses frost-client to generate signatures for given pair of (threshold, max-signers),
//! all the way from DKG, spawning subprocesses which interact with each other over the frostd
//! server. through the frost-client. Finally we read the generated signature and verify that it
...
/// We sign the `message.json` found in the frost-client signing example
fn sign(pids: &[Pid], group_pk: &str, threshold: usize) -> Result<()> {
    let mut children = Vec::new();
...
/// now verify the signature generated by frost-client using the `mina-signer` crate
...
fn cross_package_test(threshold: usize, max_signers: usize) -> Result<()> {
    ...
    sign(&pids, &group_pk_hex, threshold)?;
    parse_and_verify(&group_pk_mina);
